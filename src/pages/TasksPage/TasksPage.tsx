...
const TasksPage: React.FC = () => {
  ...
  useEffect(() => {
    console.log('🔍 TasksPage: Chargement localStorage...');
    const saved = localStorage.getItem('smartpm_estimations');
    console.log(' TasksPage: Valeur brute:', saved);
    ...
    if (saved) {
      try {
        ...
        console.error('❌ TasksPage: Erreur parsing:', e);
      }
    }
  }, []);
  ...
  // ✅ 3. FONCTION D'ESTIMATION (avec logs de debug)
    ...
    try {
      ...
      if (estimationData) {
        setEstimations(prev => {
          ...
          console.log('🚀 TasksPage: Nouvel état estimations:', updated);
          return updated;
        });
        showMessage('Estimation IA générée et sauvegardée !');
      } else {
        console.error('️ TasksPage: Aucune donnée trouvée');
        showMessage('Erreur: Réponse du serveur invalide', 3000, 'error');
      }
    } catch (err: any) {
      console.warn('⚠️ TasksPage: API indisponible, mode local');
